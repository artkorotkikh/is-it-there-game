...
State and input are sent at most 20 times/s. Input sequence numbers reject reordered controls; 350 ms silence returns movement/reel/push to neutral. Discrete actions have a monotonically increasing sequence and run at most once. A round `epoch` rejects delayed messages from a previous restart. Host actions use the same simulation API as guest actions. The guest buffers up to 12 snapshots, rejects stale ticks and interpolates 100 ms behind receipt time; it does not extrapolate after loss. This adds visible guest control latency that must be assessed on real networks.

Packets are bounded to 48,000 characters and channel backlog to 96,000 bytes. State sends are skipped above that backlog; reliable overflow ends the connection visibly. Full snapshots repeat player/prop ownership, transforms, vehicle/wheels, modules/capabilities, winch, recovery and progress. Descriptions/prompts are plain text. Network snapshots are structurally checked before rendering. This is cooperative host authority, not competitive anti-cheat.
...
Release 0.7.0 is published on the existing nano-tema engine. The rooms canister is `zsua7-paaaa-aaabz-aadfq-cai`; the frontend remains `z4wnx-uqaaa-aaabz-aadeq-cai` and records remains `zvvgl-cyaaa-aaabz-aadfa-cai` at 0.6.0. The deployed frontend cookie and settings contain the verified rooms ID. This release uses direct ICE only; it does not establish TURN fallback or connectivity across arbitrary networks.
...
By default ICE has no external service, suitable for direct/local tests. To provide internet connectivity, set build value `VITE_ICE_CONFIG_PATH` to an actual same-origin endpoint such as `/connection-config`. The endpoint must return `{iceServers:[{urls:["turn:…"],username:"…",credential:"…"}]}` using short-lived credentials; optional `iceTransportPolicy:"relay"` forces relay testing. It must be implemented/configured with the owner's chosen TURN provider. It is not supplied by the rooms actor. Do not put a long-lived provider secret in Vite, JSON assets or the canister environment. Absent that service, some network pairs cannot connect.

TURN provisioning, measured 100 ms RTT / 1% packet loss, forced-relay tests and two physical devices on separate networks are outstanding until explicitly demonstrated. Local browser success is not evidence of internet-wide readiness or an enjoyable complete cooperative run.
