...
it('snapshot interpolation discards stale states and never extrapolates physics',()=>{create();const buffer=new SnapshotBuffer(),a=s.snapshot('guest-player');s.step();const b=s.snapshot('guest-player');buffer.push(a,100);buffer.push(b,150);buffer.push(a,160);expect(buffer.sample(225)?.alpha).toBe(.5);expect(buffer.sample(500)?.alpha).toBe(1);expect(buffer.sample(500)?.current.tick).toBe(b.tick);});
