...
`src/game/interaction-visual.ts` draws selected object brackets from snapshots. `src/ui/interaction-indicator.ts` positions the target name and cycle control; it does not pick a separate client-side target.
