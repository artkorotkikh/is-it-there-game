...
## Restore centered co-op cargo-loss messages — implemented locally, 2026-09-07

The user reports that solo's centered loss announcements are absent in multiplayer. Inspection found the notice markup, `CargoLossNotice` instance and update/reset calls only in `src/main.ts`; `src/expedition.ts` had none of them. Expedition snapshots already contain cargo events. The implementation below is published with frontend 0.8.1; browser delivery acceptance remains pending.

1. Reuse the existing notice model, English capability/recovery copy and `.cargo-loss` styling in expedition play. Share the small DOM presentation with solo to prevent drift. Both host and guest, including a passenger or player on foot, should see the shared bus's physical WINCH/CYCLES/SKILLS ejections. Include Explore on your own. Preserve the transparent centered three-second fade/zoom, reduced motion, accessibility and nonblocking controls. Initial loose cargo and manual removal/drop stay quiet; the newest loss replaces the previous notice, and docking the lost module dismisses it.
2. Consume authoritative `Snapshot.events` with sequence deduplication. Advance host/exploration notices on simulation ticks; explicitly implement guest reading time while gameplay is active, independent of the absence of guest physics ticks. Repeated snapshots must neither restart the notice nor freeze its expiry. Keep event/state selection consistent with guest playback so stale interpolation cannot restore an already-dismissed notice. Check dropped/reordered snapshots and the existing 20-event history; do not promise lossless event delivery from that bounded unreliable history. Add reliable delivery only if the focused evidence requires it, documenting any protocol/version change then.
3. Validate network event kinds, canister types and safe integer sequences before using them for copy lookup; `validSnapshot` currently accepts arbitrary strings for kind/type. Handle lifecycle explicitly: hide and freeze on local/shared/visibility pause or connection stall; resume the remaining reading time. Clear notice and deduplication state on a new expedition/restart (including the guest's accepted new `epoch`), leaving and disconnect cleanup. Reject old-epoch events and prevent replay of an earlier expedition's losses. Ordinary resume must not reset event consumption.
4. Extend focused headless notice/network tests for all three module types, duplicate/stale/skipped snapshots, repeat ejection after docking, guest expiry, pause/resume and restart with sequence numbers reused. Keep existing real-physics ejection tests. Run one targeted local two-account WebRTC browser flow: produce a physical loss, observe the matching centered warning on both peers, check pause/resume and restart. Inspect only the affected narrow layout with a labelled presentation fixture if needed; do not replay full delivery routes or a browser matrix. A fixture is not proof of physical ejection or network delivery.
5. Update the canonical development/networking descriptions and status with actual evidence and remaining limitations. Run the focused tests, `npm run build` (includes strict TypeScript) and `npm run docs:check`. Acceptance requires observed host and guest messages; code inspection alone is insufficient. No physics tuning, records/backend changes or deployment are needed for this planned UI integration.
